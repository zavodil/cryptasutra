...
Это перезапуск программы делегирования токенов в сети NEAR со следующими целями: 
...
* разработчики NEAR Protocol получат обратную связь и зададут импульс для развития нового типа DeFi продуктов, а также роста сообщества
...
Установите [near-shell](https://github.com/nearprotocol/near-shell) \([инструкция на русском языке](https://nodes.cryptasutra.com/near-protocol/staking#nastroika-okruzheniya)\), это приложение на Node.js, которое создает ключи доступа, подключается к платформе NEAR и отправляет транзакции по сети. Не обязательно устанавливать его на том же сервере, что и ноду, даже наоборот, лучше использовать другой компьютер.

1. Перейдите в рабочую директорию, например, `cd ~`
...
6. Запустите `near login` и следуйте инструкциям по подключению ноды \([инструкция на русском языке](https://nodes.cryptasutra.com/near-protocol/staking#otpravka-tranzakcii-na-sozdanie-steikinga)\)
7. Сразу после аутентификации `near-shell` готов принимать команды по открытию стейкинга
...
Перед тем как начать, убедитесь, что ваш сервер обладает общедоступным ip-адресом. Запустите [nearup по пошаговой инструкции](https://github.com/near/nearup) \([инструкция на русском языке](https://nodes.cryptasutra.com/near-protocol/nearup)\) и запустите ноду с помощью образа в докере или исходников `nearcore`.
...
Stake for user 'zavodil.betanet' with 'ed25519:8qfcviFnPucrMvN19f1hoVKhhtntGjosRHNJzaXiRem7'
Starting NEAR client docker...
Node is running! 
To check logs call: docker logs --follow nearcore
```

По данному тексту можно удостовериться, что для юзера `zavodil.betanet` был автоматически сгенерированы ключи и получен публичный ключ `ed25519:8qfcviFnPucrMvN19f1hoVKhhtntGjosRHNJzaXiRem7`
...
Вы можете получить следующую ошибку, если забудете указать, что нода должна обращаться к `betanet` :
...
Пользователи смогут делегировать свои токены на ноды валидаторов как только соответствующий смарт-контракт будет опубликован в сети.
...
1. Через 2 эпохи \(24 часа\), вы увидите, что ваша нода присутствует в логах со статусом `V/n` , которая означает статус валидатора:
...
* next\_fishermen \(следующие рыбаки\)
* **current\_proposals \(текущие заявки\)** **&lt;&lt;  ваша транзакция должна быть тут**
* prev\_epoch\_kickout \(исключены за прошлую эпохи\)

Обратите внимание, что каждая запись в объекте `current_validators` содержит ожидаемый аптайм:
...
Чтобы остановить ноду, достаточно выполнить команду:
